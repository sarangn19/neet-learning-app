...
import type { Grade, Module, Chapter } from '../types';
...
function ModuleCard({ module, subjectId, grade, chapterId, chapter, lessonProgress, index }: {
  module: Module;
  subjectId: string;
  grade: Grade;
  chapterId: string;
  chapter: Chapter;
  lessonProgress: Record<string, { completed: boolean; stars: number }>;
  index: number;
}) {
  ...
  // Fix: Check if previous module is fully completed
  const prevModule = index > 0 ? chapter.modules[index - 1] : null;
  const prevModuleCompleted = prevModule
    ? prevModule.levels.every(l => lessonProgress[l.id]?.completed)
    : true;
